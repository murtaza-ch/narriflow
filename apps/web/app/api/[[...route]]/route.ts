...
import { motionDocumentExportError } from "@/lib/motion-document-mutation";
...
app.post("/webhooks/tiktok/publication", async (c) => {
	...
	try {
		...
		if (error instanceof TikTokPublicationWebhookError) {
			const status =
				error.code === "tiktok_webhook_payload_invalid" ? 400 : 401;
...
app.route(
	...
	createWorkspaceBillingHttpRoutes({
		...
		observeCheckoutReturn: (input) =>
			billingService.observeCheckoutReturn(input),
		openPortal: (input) => billingService.openPortal(input),
		readBillingState: (workspaceId) =>
			billingService.readBillingState(workspaceId),
...
app.get("/projects", async (c) => {
	...
	const source = ["all", "youtube", "link", "upload", "rss"].includes(
		...
		: undefined;
	const sort = ["newest", "oldest", "title", "clips"].includes(
		sortRaw ?? "newest",
	)
...
app.get("/workspace/exports/:exportId/download", async (c) => {
	...
	if (!variant?.downloadUrl) {
		return c.json(
			{ error: "export_file_not_ready", message: "Export file is not ready" },
			409,
		);
	}
...
app.patch("/workspace/avatar", async (c) => {
	...
	const payload = await c.req.json().catch(() => ({}));
	const storageKey =
		payload.storageKey === null ? null : String(payload.storageKey ?? "");
...
app.get("/projects/:id/runs/:workflowRunId", async (c) => {
	...
	if (!snapshot.run) {
		return c.json(
			{ error: "workflow_run_not_found", message: "Workflow run not found" },
			404,
		);
	}
...
app.get("/projects/:id/transcript", async (c) => {
	...
	if (!transcript) {
		return c.json(
			{ error: "transcript_not_found", message: "Transcript not found" },
			404,
		);
	}
...
app.post("/projects/:id/assisted-copy/generations", async (c) => {
	...
	return c.json(
		await assistedSocialCopyService.generate({
			actorUserId: appUser.actorUserId,
			workspaceId: appUser.workspaceId,
			projectId,
			...body,
...
app.post("/projects/:id/thumbnail-frames", async (c) => {
	...
	return c.json(
		await thumbnailFramePreparationService.request({
			actorUserId: appUser.actorUserId,
			workspaceId: appUser.workspaceId,
			projectId,
			...body,
...
app.get("/projects/:id/thumbnail-frames/:operationId", async (c) => {
	...
	return c.json(
		await thumbnailFramePreparationService.get(
			appUser.workspaceId,
			projectId,
			operationId,
		),
...
app.post("/projects/:id/thumbnail-frames/:operationId/retry", async (c) => {
	...
	return c.json(
		await thumbnailFramePreparationService.retry(
			appUser.workspaceId,
			projectId,
			operationId,
		),
...
app.post(
	"/projects/:id/campaign-operations/preview-editor-action",
	async (c) => {
		const appUser = authenticatedHonoActor(c);
		const { id: projectId, body } = authenticatedHonoInput<{
			id: string;
...
app.get("/projects/:id/export-bundles/:bundleId", async (c) => {
	const appUser = authenticatedHonoActor(c);
	const { id: projectId, bundleId } = authenticatedHonoInput<{
		id: string;
		bundleId: string;
	}>(c);
	return c.json(
...
app.post(
	"/projects/:id/campaign-operations/:operationId/retry-export-bundle",
	async (c) => {
		const appUser = authenticatedHonoActor(c);
		const {
			id: projectId,
...
app.post(
	"/projects/:id/brand-profiles/:profileId/scene-templates/:templateId/apply",
	async (c) => {
		const appUser = authenticatedHonoActor(c);
		const { id, profileId, templateId, body, idempotencyKey, retryOfId } =
			authenticatedHonoInput<{
				id: string;
				profileId: string;
				templateId: string;
...
app.post("/projects/:id/review-rounds", async (c) => {
	...
	const { id, body } = authenticatedHonoInput<{
		id: string;
		body: CreateReviewRoundInput;
	}>(c);
	return c.json(
		await reviewService.createRound(
			{
				actorUserId: appUser.actorUserId,
				workspaceId: appUser.workspaceId,
				projectId: id,
...
app.get("/projects/:id/review-rounds", async (c) => {
	...
	const access = workspaceAllowsCapability(
		...
		: "view";
	return c.json(
		await reviewService.internalRoom(appUser.workspaceId, id, access),
		200,
	);
});
...
app.post("/projects/:id/review-rounds/:roundId/invite", async (c) => {
	...
	return c.json(
		await reviewService.inviteReviewers(
			{
				actorUserId: appUser.actorUserId,
				workspaceId: appUser.workspaceId,
				projectId: id,
...
app.post(
	"/projects/:id/review-rounds/:roundId/notifications/retry",
	async (c) => {
		const appUser = authenticatedHonoActor(c);
		const { id, roundId, body } = authenticatedHonoInput<{
			id: string;
			roundId: string;
			body: RetryReviewNotificationInput;
		}>(c);
		...
		return result.retrying
			...
			: c.json(
					{
						error: "review_notification_not_found",
						message: "Failed notification was not found",
					},
					404,
				);
	},
...
app.post("/projects/:id/review-rounds/:roundId/revoke", async (c) => {
	const appUser = authenticatedHonoActor(c);
	const { id, roundId } = authenticatedHonoInput<{
		id: string;
		roundId: string;
	}>(c);
	return c.json(
...
app.post("/projects/:id/clips/:clipId/exports", async (c) => {
	...
	const staleCount = editor.document.censorSegments.filter(
		(segment: CensorSegment) =>
			segment.enabled &&
			isCensorSegmentStale(segment, editor.document.transcriptSlice),
	).length;
	...
	if (staleCount > 0) {
		await autoCensorService
			.recordEvent(appUser, c.req.param("id"), c.req.param("clipId"), {
				type: "auto_censor_export_notice",
				staleCount,
			})
			.catch(() => undefined);
	...
	return c.json(
		result,
		result.reused && result.export.status === "ready" ? 200 : 202,
	);
});
...
app.get("/social/oauth/start/:platform", async (c) => {
	...
	if (!parsedPlatform.success) {
		return c.json(
			{ error: "invalid_input", message: "Unsupported social platform" },
			400,
		);
	}
...
app.get("/social/oauth/callback", async (c) => {
	...
	try {
		...
		if (result.facebookSelectionToken)
			successRedirect.searchParams.set(
				"facebook_selection",
				result.facebookSelectionToken,
			);
		else
			successRedirect.searchParams.set(
				"connected",
				String(result.accounts.length),
			);
...
app.get("/social/oauth/facebook-selection/:token", async (c) => {
	...
	if (!/^[A-Za-z0-9_-]{32,128}$/.test(token))
		return c.json({ error: "social_facebook_selection_invalid" }, 400);
	try {
		return c.json(
			{
		...
		const code =
			error instanceof SocialOAuthError
				? error.code
				: "social_facebook_selection_failed";
		return c.json(
			{
				error: code,
				message:
		...
		return c.json(
			{
				...
				message:
					error instanceof SocialOAuthError
						? error.message
						: "Facebook Page could not be connected",
			},
			400,
		);
	}
...
app.get("/audio-assets", async (c) => {
	...
	const result = await audioAssetService.listAssets(
		appUser.workspaceOwnerUserId,
		parsed.data,
		{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
	);
...
app.post("/audio-assets/presign-upload", async (c) => {
	...
	const result = await audioAssetService.presignUpload(
		appUser.workspaceOwnerUserId,
		parsed.data,
		{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
	);
...
app.post("/audio-assets", async (c) => {
	...
	const asset = await audioAssetService.finalizeUpload(
		appUser.workspaceOwnerUserId,
		parsed.data,
		{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
	);
...
app.get("/audio-assets/:id/playback-url", async (c) => {
	...
	if (!idParsed.success) {
		return c.json(
			{ error: "invalid_audio_asset_id", message: "Invalid audio asset id" },
			400,
		);
	}
	const source = await audioAssetService.getPlaybackSource(
		appUser.workspaceOwnerUserId,
		idParsed.data,
		{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
	);
...
app.put("/audio-assets/:id/favorite", async (c) => {
	...
	return c.json(
		await audioAssetService.setFavorite(
			appUser.workspaceOwnerUserId,
			idParsed.data,
			true,
			{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
...
app.delete("/audio-assets/:id", async (c) => {
	...
	await audioAssetService.deleteUserAsset(
		appUser.workspaceOwnerUserId,
		idParsed.data,
		{ workspaceId: appUser.workspaceId, actorUserId: appUser.actorUserId },
	);
