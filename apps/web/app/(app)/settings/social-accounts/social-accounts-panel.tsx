...
function connectHref(platform: SocialPlatform, returnTo?: string) {
	if (returnTo)
		return `/api/social/oauth/start/${platform}?redirect=${encodeURIComponent(returnTo)}`;
...
export function SocialAccountsPanel({
	...
	useEffect(() => {
		...
		fetch(
			`/api/social/oauth/facebook-selection/${encodeURIComponent(facebookSelectionToken)}`,
			{ cache: "no-store" },
		)
			.then(async (response) => {
				...
				if (!response.ok)
					throw new Error(
						payload.message || "Facebook Pages could not be loaded",
					);
	...
	async function selectFacebookPage(pageId: string) {
		...
		try {
			...
			if (!response.ok)
				throw new Error(
					payload.message || "Facebook Page could not be connected",
				);
			toaster.create({
				type: "success",
				title: "Facebook Page connected",
				description: payload.account.displayName,
			});
			router.replace("/settings/social-accounts", { scroll: false });
			...
			toaster.create({
				type: "error",
				title: "Could not connect Facebook Page",
				description:
					reason instanceof Error
						? reason.message
						: "Try connecting Facebook again.",
		...
		try {
			...
			if (!response.ok) {
				...
				console.warn(
					JSON.stringify({
						level: "error",
						message: "social_disconnect_failed",
		...
		<Box as="section">
			{facebookSelectionToken && facebookPages.length > 0 ? (
				<Box mb="6" borderTopWidth="3px" borderColor="accent.solid" pt="4">
					...
					<Text mt="1" fontSize="13px" color="fg.muted">
						Only the Page you choose will be connected to this workspace.
					</Text>
						...
						{facebookPages.map((page) => (
							...
							>
								<SocialAvatar
									...
									size="8"
								/>
								<Text flex="1" fontSize="13px" fontWeight="600">
								...
								>
									{selectingPage === page.id ? "Connecting…" : "Connect"}
								</Button>
							</Flex>
						))}
					</Stack>
							...
							>
								<Box position="relative" flexShrink={0}>
									...
									<Flex
										...
										role="img"
										aria-label={platformLabels[account.platform]}
								...
								{canManage && account.status !== "active" ? (
									<Button variant="outline" size="sm" asChild>
										<Link href={connectHref(account.platform, returnTo)}>
											Reconnect
										</Link>
									</Button>
								) : null}
			...
			<Stack gap="2" mb="5" maxW="560px">
				...
				<Text fontSize="13px" color="fg.muted">
					{canManage
						? "Sign in to the social account you want to add, then choose a platform below."
						: "Ask a workspace owner or admin to connect a social account."}
				</Text>
			</Stack>
			<SimpleGrid
				columns={{ base: 1, sm: 2, md: 3 }}
			...
			>
				{platforms.map((platform) => {
					...
					return canManage ? (
						...
						>
							<Link
								href={connectHref(platform, returnTo)}
								aria-label={`Connect ${platformLabels[platform]} account`}
							>
								{content}
							</Link>
						</Box>
