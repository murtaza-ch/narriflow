  ...
  return (
    <Stack gap="7">
      <Grid templateColumns={{ base: "1fr", xl: "360px 1fr" }} gap="8" alignItems="start">
        ...
        {scenes.length === 0 ? (
          <Box  bg="bg.panel" borderWidth="1px" borderRadius="l2" borderColor="border" p="8"><Text textStyle="title" fontSize="20px">No reusable scenes yet</Text><Text mt="2" fontSize="13px" color="fg.muted">Start with an intro, transition card, or closing call to action.</Text></Box>
        ) : (
          <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap="4">
            {scenes.map((scene, index) => (
              <Stack key={scene.id} gap="4" bg="bg.panel" borderWidth="1px" borderRadius="l2" borderColor={scene.isDefault ? "border.accent" : "border"} p="4">
