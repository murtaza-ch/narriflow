    ...
    <Stack gap="7">
      <Grid templateColumns={{ base: "1fr", xl: "360px 1fr" }} gap="8" alignItems="start">
        <Stack gap="4" borderTopWidth="3px" borderColor="accent.solid" pt="5">
          ...
          <Button colorPalette="accent" disabled={!name.trim() || !text.trim() || busy !== null} onClick={create}><Plus size={15} />Save scene</Button>
          {error ? <Text color="danger.fg" fontSize="12px">{error}</Text> : null}
        </Stack>

        {scenes.length === 0 ? (
          <Box layerStyle="blueprint" borderTopWidth="1px" borderBottomWidth="1px" borderColor="border" p="8"><Text textStyle="title" fontSize="20px">No reusable scenes yet</Text><Text mt="2" fontSize="13px" color="fg.muted">Start with an intro, transition card, or closing call to action.</Text></Box>
        ) : (
          <Grid templateColumns={{ base: "1fr", md: "repeat(2, 1fr)" }} gap="4">
            {scenes.map((scene, index) => (
              <Stack key={scene.id} gap="4" borderTopWidth="1px" borderBottomWidth="1px" borderColor={scene.isDefault ? "border.accent" : "border"} p="4" position="relative" _before={{ content: '""', position: "absolute", insetInlineStart: 0, top: 0, bottom: 0, w: "3px", bg: scene.isDefault ? "accent.solid" : "border.control" }}>
                ...
                <Flex justify="space-between" align="center"><Box><Text fontSize="13px" fontWeight="650">{scene.name}</Text><Text textStyle="data" color="fg.timecode" fontSize="10px">{scene.definition.durationSec.toFixed(1)}s · r{scene.revision}</Text></Box><Flex gap="1">{canManageDefaults && scene.role !== "inline" && !scene.isDefault ? <Button size="xs" variant="ghost" aria-label={`Make ${scene.name} default`} disabled={busy !== null} onClick={() => request(scene.id, `/api/brand-profiles/${profileId}/scene-templates/${scene.id}`, { method: "PATCH", body: JSON.stringify({ expectedRevision: scene.revision, makeDefault: true }) })}><Check size={13} /></Button> : null}<Button size="xs" variant="ghost" color="danger.fg" aria-label={`Delete ${scene.name}`} disabled={busy !== null} onClick={() => request(scene.id, `/api/brand-profiles/${profileId}/scene-templates/${scene.id}`, { method: "DELETE", body: JSON.stringify({ expectedRevision: scene.revision }) })}><Trash2 size={13} /></Button></Flex></Flex>
              </Stack>
            ))}
          </Grid>
        )}
