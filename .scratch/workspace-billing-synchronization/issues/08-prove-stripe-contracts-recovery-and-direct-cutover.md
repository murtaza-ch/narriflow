...
- Stripe uses pinned API version `2026-07-29.dahlia`; async raw-body signature fixtures cover every registered event. The production adapter contract proves complete pagination and every supported subscription status, while a real test-mode contract covers idempotent customer/Checkout recovery, expiration, configured portal, trialing/incomplete/active current-state retrieval, cancellation-at-period-end, prorated seat create/update/delete, and isolated cleanup.
...
- The disposable PostgreSQL drill applies the full chain through typed Checkout outcomes and passes representative pending, paid, past-due, and conflicted fixtures, account/provider uniqueness, concurrent Checkout, duplicate delivery, atomic projection/retention/audit, fairness beyond three batches, lease takeover, stale fencing, and transaction rollback.
- Bounded metrics cover delivery disposition, claim source, queue age, settlement outcome, operation duration, and desired/synchronized seat quantity without Workspace or provider identifiers.
