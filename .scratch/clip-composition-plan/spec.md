...
Creators use Studio to decide how a clip will look and sound, then trust Narriflow to export that composition. Some parity rules are already shared, including caption cues, framing-mode precedence, logo settings, music fades, ducking windows, and speaker overrides. The composition itself is still interpreted separately by Studio and the render worker.

That split is visible today. Split preview uses fixed left and right crops while export uses detected, per-scene speaker geometry. Screen preview and export duplicate picture-in-picture fitting rules, including the same 8 percent margin and 40 percent minimum-width gate. A clip with B-roll and an explicit split or screen layout can preview as stacked outside the cutaway even though the worker falls back the whole clip to single-speaker framing. Audio-only background behavior also differs. Several expected fallbacks are silent, so a creator can see a confident preview without knowing that export will choose another composition.

The duplication also costs compute and maintenance time. Explicit split rendering can repeat face analysis even when compatible automatic layout analysis already exists. Screen rendering can repeat face confirmation after picture-in-picture evidence has been persisted. The worker resolves routing and geometry separately for each output, while Studio recomputes the active scene and crop geometry on every playback update. Each new layout or fallback must be implemented and tested twice, and the two implementations can drift on rounding, precedence, timing, or resource use.
...
Migration is additive. The new planner first runs beside current behavior and compares canonical plan decisions without changing visible output. Composition modes then move through both adapters one complete slice at a time. Existing render topology, codecs, quality, lifecycle ownership, editor ownership, and optional-asset degradation remain compatible until a mode has plan-interface tests, adapter contracts, real-media evidence, browser verification, and resource measurements. Only then is the corresponding legacy policy removed.
...
36. As an operator, I want plain multi-output compositions to keep the existing shared encode path, so that deepening the module does not multiply source decodes.
...
45. As a maintainer, I want legacy and new decisions compared in shadow mode, so that migration mismatches are found before users see them.
46. As a maintainer, I want each composition mode cut over independently, so that rollout and rollback do not require a risky all-at-once rewrite.
...
- A valid durable automatic layout analysis is the preferred speaker-scene evidence for Auto and for any explicit Split behavior proven compatible in shadow comparison. The existing explicit Split detector remains available during migration and rollback until representative fixtures prove the shared evidence preserves or improves the approved behavior.
- A valid durable screen analysis is reused according to its input fingerprint. Repeated face confirmation may be eliminated only after the evidence version records the confirmation inputs needed to make reuse deterministic. Until then, the current guard remains, and the plan reports that exact screen framing is pending or approximate.
...
- Existing B-roll precedence remains compatible during cutover. Automatic speaker scenes may compose below cutaways as they do today. Explicit Split or Screen with active B-roll uses the current whole-clip single-speaker fallback until a separately approved composition rule changes that behavior. Studio must preview and explain the same effective result.
...
- Plain targets retain the current shared multi-output encode topology. The FFmpeg adapter may group additional structurally equivalent targets only when a measured pixel, branch, scene, and memory budget permits it. Segment-heavy plans keep the safer per-output topology by default. This work does not raise render concurrency or scene caps.
...
- Migration uses expand and contract. Add the planner and adapters beside existing behavior, generate plans in shadow mode, compare canonical decisions, move deterministic modes first, migrate scene-aware modes and audio next, then delete copied web and worker policy only after their replacement tests pass.
- Cutover controls are mode-specific and read once from validated worker and web configuration. Rolling versions may coexist because the plan is not persisted, but an adapter must reject an unknown plan version before side effects. Rollback selects the retained legacy mode implementation until final contraction.
- The existing shared multi-output, per-output edited, B-roll, screen, split, automatic layout, and audiogram output topologies remain available throughout migration. No ticket may remove a topology before equivalent plan-interface, adapter, real-media, and browser coverage exists.
...
- Real-FFmpeg tests render representative keyframes and audio windows for each topology and compare media probes plus approved frame and audio tolerances with the legacy path. Byte identity is required where the adapter command is intended to remain unchanged. Geometry and perceptual equivalence are required where command construction changes without an approved visual change.
...
- Performance tests use representative plain, edited, automatic, split, screen, B-roll, and four-target fixtures. They assert analysis and extraction call counts, retain the plain shared-encode path, enforce command grouping budgets, and record peak RSS before any broader grouping is enabled.
- Shadow-mode tests compare requested and effective mode, scene boundaries, layer topology, target geometry, fallback reason, audio schedule, and adapter topology. Any unexplained mismatch blocks that mode's cutover.
- Final verification for every implementation ticket runs uncached focused tests, affected database tests when evidence persistence changes, repository typecheck, lint, and production build. Mode cutover also requires a real-browser checklist and representative real-media output inspection.
