...
function provider(
  generate: AssistedCopyProvider["generate"] = async ({ platforms }) => ({
    ...
    variants: platforms.map((platform) => ({
      platform,
      caption: platform === "x" ? "A precise launch note." : "A precise launch note with context.",
      hashtags: ["#Narriflow"],
...
function moduleWith(overrides: {
  ...
  return createAssistedSocialCopy({
    ...
    loadContext: overrides.loadContext ?? (async () => ({
    ...
    moderate: overrides.moderate ?? (async () => ({ outcome: "approved" as const })),
...
describe("assisted social copy", () => {
  ...
  test("rejects the whole generation when moderation rejects one result", async () => {
    const copy = moduleWith({
      moderate: async ({ platform }) => ({
        outcome: platform === "x" ? "rejected" as const : "approved" as const,
  ...
  test("regeneration must preserve every locked phrase and hashtag", async () => {
    const copy = moduleWith({
      provider: provider(async ({ platforms, lockedPhrases, lockedHashtags }) => ({
        model: "copy-test-model",
        inputTokens: 80,
        outputTokens: 40,
        variants: platforms.map((platform) => ({
          ...
          title: platform === "youtube_shorts" ? lockedPhrases.join(" ") : null,
        })),
      })),
    });
    ...
    await expect(invalid.generate({
      ...BASE_INPUT,
      lockedPhrases: ["Launch week"],
    })).rejects.toMatchObject({ code: "assisted_copy_locked_content_missing" });
  });

  test("stores user edits and confirms their exact fingerprint", async () => {
    const copy = moduleWith();
    const generated = await copy.generate(BASE_INPUT);
    const variant = generated.variants[0]!;

    const confirmed = await copy.confirm({
      actorUserId: BASE_INPUT.actorUserId,
      workspaceId: BASE_INPUT.workspaceId,
      projectId: BASE_INPUT.projectId,
      variantId: variant.id,
      caption: "The editor's final wording.",
      hashtags: ["#Final"],
      title: "Final title",
    });
    ...
    await expect(copy.requirePublicationConfirmation({
      workspaceId: BASE_INPUT.workspaceId,
      projectId: BASE_INPUT.projectId,
      clipId: BASE_INPUT.clipId,
      platform: "youtube_shorts",
      variantId: variant.id,
      publishedCaption: "The editor's final wording.\n\n#Final",
      title: "Final title",
    })).resolves.toMatchObject({ confirmed: true });
    await expect(copy.requireConfirmation({
      workspaceId: BASE_INPUT.workspaceId,
      projectId: BASE_INPUT.projectId,
      clipId: BASE_INPUT.clipId,
      platform: "youtube_shorts",
      variantId: variant.id,
      caption: "Changed after confirmation.",
      hashtags: ["#Final"],
      title: "Final title",
    })).rejects.toMatchObject({ code: "assisted_copy_confirmation_stale" });
  });
  ...
  test("replays duplicate submission and rejects key reuse with new input", async () => {
    ...
    expect(calls).toBe(1);
    await expect(copy.generate({ ...BASE_INPUT, campaignNote: "Different" }))
      .rejects.toBeInstanceOf(AssistedSocialCopyError);
  });

  test("validates the composed caption and scopes confirmation to the project", async () => {
    const copy = moduleWith({
    ...
    await expect(copy.generate({ ...BASE_INPUT, platforms: ["x"] }))
