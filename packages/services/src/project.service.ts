...
export interface ClaimedClipRenderAttempt
	extends Omit<ClaimedWorkflowRun, "stage" | "lifecycleVersion" | "attemptId"> {
...
export class ProjectService {
	...
	> {
		const prisma = this.requirePrisma();
		const waitingV2 =
			await getWorkflowRunLifecycle().claimDueWaitingTranscripts(
				batchSize,
				minPollIntervalMs,
			);
	...
	async completeTranscriptWorkflowRun(
		...
		if (options?.expectedProviderJobId) {
			...
			if (currentTranscript?.providerJobId !== options.expectedProviderJobId) {
				console.warn(
					JSON.stringify({
						level: "warn",
	...
	private async failOrphanedRenderingVariants(
		...
		if (exportVariantIds.length > 0) {
			...
			for (const exportId of new Set(
				affected.map((variant) => variant.exportId),
			)) {
