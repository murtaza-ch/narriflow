...
describe("runProjectDeletion", () => {
	function baseRow(
		overrides: Partial<ProjectDeletionRow> = {},
	): ProjectDeletionRow {
		return {
	...
	test("publication guard preserves storage for live or uncertain provider work", async () => {
		const deps = makeDeps({
			access: "owned",
			row: baseRow({ hasActivePublication: true }),
		});

		const outcome = await runProjectDeletion(deps);

		expect(outcome).toEqual({ kind: "active_publication" });
		expect(deps.calls.deleteObject).toEqual([]);
		expect(deps.calls.deleteProjectRow).toBe(0);
	});
