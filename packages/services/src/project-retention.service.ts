...
export function getRetentionRuntimeConfig(
	...
	return {
		mode,
		activationAt: parsed && Number.isFinite(parsed.getTime()) ? parsed : null,
	};
}
...
export class ProjectRetentionService {
	...
	async applyWorkspaceTierTransition(
		...
		if (transition.kind === "clear_unexpired") {
			const rescueCutoff =
				input.observedAt &&
				input.observedAt.getTime() > input.effectiveAt.getTime()
					? input.observedAt
					: input.effectiveAt;
	...
	async getObserveMetrics(
		now = new Date(),
	): Promise<RetentionObserveMetrics | null> {
		const prisma = getPrismaClient();
		const config = getRetentionRuntimeConfig();
	...
	async purgeDueProjects(
		...
		for (let index = 0; index < Math.max(1, Math.min(100, limit)); index += 1) {
			...
			try {
				...
				summary.failed += 1;
				await this.recordPurgeFailure(
					claimed.id,
					claimed.purgeAttemptCount,
					error,
				);
			}
		}
	...
	private async purgeClaimedProject(
		...
		const deleteAfter = new Date(
			completedAt.getTime() + 90 * 24 * 60 * 60 * 1000,
		);
		const receipt = {
			version: 1,
			projectIdHash: hash,
