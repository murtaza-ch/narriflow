...
describe("thumbnail frame extraction", () => {
  test("renews the job and exact cleanup hold during processing and immediately before upload and adoption", async () => {
    ...
    let extractionStarted: (() => void) | null = null;
    ...
    const baseStore = createInMemoryThumbnailExtractionStore({
      variants: [{
        id: exportVariantId,
        workspaceId: scope.workspaceId,
        projectId: scope.projectId,
    ...
    const store: typeof baseStore = {
      ...
      async renewOutput(input) {
        events.push(`renew:${input.now.toISOString()}`);
        return baseStore.renewOutput(input);
      },
      async complete(input) {
        events.push("adopt");
        return baseStore.complete(input);
      },
    };
    ...
    await started;
    currentTime = new Date("2026-08-31T10:01:00.000Z");
    await runHeartbeat?.();
    expect(store.outputObligations()[0]?.claimExpiresAt).toEqual(
      new Date("2026-08-31T10:03:00.000Z"),
    );
    currentTime = new Date("2026-08-31T10:01:30.000Z");
    releaseExtraction?.();

    await expect(processing).resolves.toMatchObject({ status: "completed" });
    expect(events).toEqual([
      "renew:2026-08-31T10:01:00.000Z",
      "renew:2026-08-31T10:01:30.000Z",
      "upload",
      "renew:2026-08-31T10:01:30.000Z",
      "adopt",
    ]);
  ...
  test("aborts frame processing when atomic job and cleanup renewal loses ownership", async () => {
    let runHeartbeat: (() => Promise<void>) | null = null;
    let extractionStarted: (() => void) | null = null;
    const started = new Promise<void>((resolve) => {
      extractionStarted = resolve;
    ...
    await expect(processing).resolves.toMatchObject({
      status: "failed",
      errorCode: "thumbnail_claim_lost",
    });
