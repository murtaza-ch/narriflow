...
function requiredPrisma() {
  ...
  if (!prisma) throw new Error("DATABASE_URL is required for workspace library operations");
  return prisma;
}
...
export class WorkspaceLibraryService {
  ...
  async renameFolder(userId: string, workspaceId: string, folderId: string, name: string) {
    await workspaceService.requireActor(userId, workspaceId, "content.edit");
    const safeName = validFolderName(name);
    ...
    const rows = await requiredPrisma().clipExport.findMany({
      where: {
        ...
        ...(filters.status === "processing"
          ? { status: { in: ["queued", "rendering", "partial_ready"] as ClipExportStatus[] } }
          : filters.status ? { status: filters.status } : {}),
        ...(filters.projectId ? { projectId: filters.projectId } : {}),
        ...(filters.aspectRatio ? { variants: { some: { aspectRatio: filters.aspectRatio } } } : {}),
        ...
        ...(filters.query?.trim()
          ? {
              OR: [
                { project: { title: { contains: filters.query.trim(), mode: "insensitive" } } },
                { clip: { title: { contains: filters.query.trim(), mode: "insensitive" } } },
              ],
            }
          : {}),
      },
      select: {
        ...
        variants: {
          select: { id: true, aspectRatio: true, status: true, storageKey: true, sizeBytes: true },
  ...
  async listExportProjects(userId: string, workspaceId: string) {
    ...
    return requiredPrisma().project.findMany({
      where: { workspaceId, clipExports: { some: { workspaceId } }, ...accessibleProjectWhere() },
  ...
  async getCalendarFilters(userId: string, workspaceId: string) {
    ...
    const [accounts, projects] = await Promise.all([
      ...
      prisma.project.findMany({
        where: { workspaceId, socialPosts: { some: { workspaceId } }, ...accessibleProjectWhere() },
        select: { id: true, title: true },
        orderBy: { title: "asc" },
      }),
    ]);
  ...
  async getCalendarComposerOptions(userId: string, workspaceId: string) {
    await workspaceService.requireActor(userId, workspaceId, "publishing.manage");
    const prisma = requiredPrisma();
