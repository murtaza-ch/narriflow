...
import { buildBrandTemplateSnapshot } from "./brand-profile.service";
import {
...
async function resolveMemberStyleSelection(
  ...
  const style = buildBrandTemplateSnapshot(membership.template);
  ...
  async renderSelected(input: {
    ...
    try {
      operation = await prisma.campaignOperation.create({ data: {
        ...
        leaseExpiresAt: new Date(Date.now() + 10 * 60_000),
        items: { create: normalizedIds.map((clipId) => ({ itemKey: clipId, requestedClipId: clipId, clipId: byId.has(clipId) ? clipId : null, expectedEditorRevision: byId.get(clipId)?.editorRevision ?? null, status: byId.has(clipId) ? "pending" : "ineligible", errorCode: byId.has(clipId) ? null : "campaign_clip_not_found" })) },
      } });
    } catch (error) {
  ...
  async getEditorActionCatalog(
    ...
    const styles = (liveProfile?.templates ?? []).flatMap((membership) => {
      const parsed = brandTemplateSnapshotSchema.safeParse(
        buildBrandTemplateSnapshot(membership.template),
      );
