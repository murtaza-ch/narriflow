...
describe("Social Publication Attempt", () => {
	test("publishes a retained frozen thumbnail object after the Visual Asset is soft-deleted", async () => {
		let receivedThumbnail: unknown = null;
		const platform = {
			capabilities: {
				recovery: "exact" as const,
				asynchronous: false,
			...
			async publish(input: Parameters<ReturnType<typeof createDeterministicPublicationPlatform>["publish"]>[0]) {
				receivedThumbnail = input.thumbnail;
				return {
					kind: "accepted" as const,
					receipt: {
						receiptId: "provider-receipt-thumbnail",
						platformPostId: "provider-post-thumbnail",
						externalUrl: null,
						metrics: null,
					},
				};
		...
		const thumbnailSeed = {
			...
			socialPost: {
				...
				thumbnail: {
					storageKey: "visual-assets/thumbnail.jpg",
		...
		} satisfies PublicationAttemptSeed;
		const { attempt } = createAttemptHarness(platform, thumbnailSeed);
		...
		expect(receivedThumbnail).toEqual({
			storageKey: "visual-assets/thumbnail.jpg",
			fileName: "thumbnail-post-1.jpg",
	...
	test("fails frozen thumbnail evidence mismatches before any provider call", async () => {
		for (const socialPost of [
			{
				...seed.socialPost,
				thumbnailFingerprint: "a".repeat(64),
				thumbnail: {
		...
		] satisfies PublicationAttemptSeed["socialPost"][]) {
			let providerCalls = 0;
			...
			const { attempt } = createAttemptHarness(counted, { ...seed, socialPost });
			...
			await expect(attempt.execute({
				...
				kind: "failed",
				code: "publication_thumbnail_evidence_invalid",
			});
