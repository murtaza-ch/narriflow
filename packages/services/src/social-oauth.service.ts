...
const META_GRAPH_VERSION =
	SOCIAL_PROVIDER_CAPABILITIES.instagram_reels.apiVersion;
...
function decryptToken(encrypted: string) {
	...
	if (version !== "v1" || !ivRaw || !tagRaw || !ciphertextRaw) {
		throw new SocialOAuthError(
			...
			"Stored social token is not in a supported format",
		);
	}
...
export function eligibleFacebookPages(value: unknown): FacebookPageCandidate[] {
	...
	return value.data.slice(0, 100).flatMap((candidate) => {
		...
		if (
			...
			page.access_token.length > 4_096 ||
		...
		return [
			{
				...
				avatarUrl:
					typeof page.picture?.data?.url === "string" &&
					page.picture.data.url.length <= 2_048
...
export function grantedFacebookScopes(value: unknown): string[] {
	...
	return value.data.flatMap((permission) => {
		...
		return typeof item.permission === "string" && item.status === "granted"
			? [item.permission]
			: [];
	});
}
...
export class SocialOAuthService {
	...
	async createAuthorizationUrl(params: {
		...
		const verifier =
			platform === "x" || platform === "youtube_shorts"
				? randomBase64Url(48)
				: null;
	...
	async handleCallback(params: {
		...
		if (!savedState || savedState.expiresAt.getTime() < Date.now()) {
			...
			throw new SocialOAuthError(
				...
				"Social OAuth state is missing or expired",
			);
		}
		...
		if (platform === "facebook_reels") {
			...
			if (facebook.selection) {
				await prisma.socialOAuthState.update({
					...
					data: {
						codeVerifier: encryptToken(JSON.stringify(facebook.selection)),
						expiresAt: new Date(Date.now() + OAUTH_STATE_TTL_MS),
					},
				});
				return {
					...
					facebookSelectionToken: savedState.state,
				};
			}
	...
	async getFacebookPageSelection(
		...
		if (!state?.codeVerifier)
			throw new SocialOAuthError(
				"social_facebook_selection_invalid",
				"Facebook Page selection is invalid or expired",
			);
		const payload = JSON.parse(
			decryptToken(state.codeVerifier),
		) as FacebookSelectionPayload;
		return payload.pages.map(({ id, name, avatarUrl }) => ({
			id,
			name,
			avatarUrl,
		}));
	...
	async completeFacebookPageSelection(
		...
		if (!page)
			throw new SocialOAuthError(
				"social_facebook_page_invalid",
				"Selected Facebook Page is not eligible",
			);
		return prisma.$transaction(async (tx) => {
			const claimed = await tx.socialOAuthState.deleteMany({
				where: { id: state.id, codeVerifier: state.codeVerifier },
			});
			if (claimed.count !== 1)
				throw new SocialOAuthError(
					"social_facebook_selection_invalid",
	...
	async getPublishAccount(accountId: string): Promise<PublishSocialAccount> {
		...
		if (
			...
			throw new SocialOAuthError(
				...
				"Connected social account token has expired",
			);
		}
	...
	private tiktokAuthorizationUrl(state: string, redirectUri: string) {
		...
		url.searchParams.set(
			"scope",
			[
				...SOCIAL_PROVIDER_CAPABILITIES.tiktok.requiredScopes,
				"video.upload",
			].join(","),
		);
	...
	private youtubeAuthorizationUrl(
		state: string,
		redirectUri: string,
	...
	private linkedinAuthorizationUrl(state: string, redirectUri: string) {
		...
		url.searchParams.set(
			"scope",
			SOCIAL_PROVIDER_CAPABILITIES.linkedin.requiredScopes.join(" "),
		);
		url.searchParams.set("state", state);
		return url.toString();
	}

	private xAuthorizationUrl(
		state: string,
		redirectUri: string,
		verifier: string | null,
	) {
		if (!verifier) {
			throw new SocialOAuthError(
				"social_oauth_pkce_missing",
				"X OAuth requires PKCE",
			);
		}
		...
		url.searchParams.set(
			"scope",
			SOCIAL_PROVIDER_CAPABILITIES.x.requiredScopes.join(" "),
		);
		url.searchParams.set("state", state);
		url.searchParams.set("code_challenge", codeChallenge(verifier));
		url.searchParams.set("code_challenge_method", "S256");
	...
	private async connectTikTok(
		...
		if (!providerAccountId) {
			throw new SocialOAuthError(
				"social_oauth_profile_missing",
				"TikTok profile did not include an account id",
			);
		}
			...
			await this.upsertAccount(
				...
				"tiktok",
				{
					...
					metadata: {
						...
						refreshExpiresAt:
							expiresAtFromSeconds(token.refresh_expires_in)?.toISOString() ??
							null,
					},
				},
				workspaceId,
			...
			connected.push(
				await this.upsertAccount(
					...
					"instagram_reels",
					{
						...
						displayName:
							ig.name || ig.username || page.name || "Instagram account",
	...
	private async connectFacebook(
		...
		const permissions = (await readSocialProviderJson(
			await fetch(
		...
		const missingScopes =
			SOCIAL_PROVIDER_CAPABILITIES.facebook_reels.requiredScopes.filter(
				(scope) => !grantedScopes.includes(scope),
			);
		if (missingScopes.length > 0) {
			throw new SocialOAuthError(
				"social_facebook_permissions_missing",
				"Facebook did not grant every required Page publishing permission",
			);
		}
		const pages = await readSocialProviderJson(
			await fetch(
		...
		if (eligiblePages.length === 0) {
			throw new SocialOAuthError(
				"social_facebook_page_missing",
				"No Facebook Page with content publishing access was returned",
			);
		}
	...
	private async connectLinkedIn(
		...
		if (!profile.sub) {
			throw new SocialOAuthError(
				"social_oauth_profile_missing",
				"LinkedIn profile did not include a subject id",
			);
		}
	...
	private async connectX(
		...
		if (!verifier) {
			throw new SocialOAuthError(
				"social_oauth_pkce_missing",
				"X OAuth state did not include a verifier",
			);
		}
		...
		if (!user?.id) {
			throw new SocialOAuthError(
				"social_oauth_profile_missing",
				"X profile did not include a user id",
			);
		}
	...
	private async refreshAccount(accountId: string) {
		...
		if (!row?.refreshTokenEncrypted) {
			throw new SocialOAuthError(
				"social_account_refresh_missing",
				"Connected social account has no refresh token",
			);
		}
