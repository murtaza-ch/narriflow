...
dbDescribe("Vizard expansion PostgreSQL contracts", () => {
	...
	beforeAll(() => {
		...
		pool = new Pool({
			...
			max: 12,
			options: databaseSchema ? `-csearch_path=${databaseSchema}` : undefined,
		});
	...
	async function fixture(label: string) {
		...
		const user = await prisma.user.create({
			data: {
				clerkId: `vizard-${label}-${suffix}`,
				primaryEmail: `vizard-${label}-${suffix}@example.test`,
			},
		});
					...
					create: [
						{
							...
							watermark: false,
							status: "completed",
							storageKey: `fixtures/${suffix}/vertical.mp4`,
							sizeBytes: 64n,
							durationSec: 10,
							completedAt: new Date(),
						},
						{
							...
							watermark: false,
							status: "completed",
							storageKey: `fixtures/${suffix}/square.mp4`,
							sizeBytes: 32n,
							durationSec: 10,
							completedAt: new Date(),
						},
	...
	test("dubbing admission enforces render readiness and reuses an idempotent queue request", async () => {
		...
		const input = {
			...
			targetLanguageCode: "es" as const,
		...
		await expect(request("")).rejects.toMatchObject({
			code: "dubbing_idempotency_key_required",
		});
		await expect(request("before-render")).rejects.toMatchObject({
			code: "dubbing_render_required",
		});
		expect(
			await prisma.clipDub.count({ where: { projectId: current.project.id } }),
		).toBe(0);
		await prisma.workspace.update({
			where: { id: current.workspace.id },
			data: { pricingTier: "creator" },
		});
		await expect(request("creator-denied")).rejects.toMatchObject({
		...
		await prisma.clipRender.create({
			data: {
				...
				aspectRatio: "ratio_9_16",
				status: "completed",
				storageKey: "fixtures/dubbing/base.mp4",
			},
		});
		...
		expect(
			await prisma.workflowRun.count({
				where: { projectId: current.project.id, stage: "dubbing" },
			}),
		).toBe(1);
		await prisma.clipDub.update({
			where: { id: first.dub.id },
			...
			const sender = new AutopilotService({
				fetchFeed: async () => {
					...
					return {
						...
						etag: "late",
						lastModified: null,
						finalUrl: rule.rssUrl,
						notModified: outcome === "not_modified",
					};
				},
			});
			try {
				expect(await sender.processDueRules(1)).toMatchObject({
					checked: 1,
					imported: 0,
					claimLost: 1,
				});
				...
				expect(
					warnings.mock.calls.map(([value]) => JSON.parse(value)),
				).toContainEqual(
					expect.objectContaining({
						message: "autopilot_claim_lost",
						ruleId: rule.id,
					}),
				);
			} finally {
				warnings.mockRestore();
				await prisma.autopilotRule.delete({ where: { id: rule.id } });
			}
		});
	}
			...
			test(`Campaign Render selected reports ${loss} claims on ${outcome}`, async () => {
				...
				const request = {
					...
					execute: async () => {
						...
						if (outcome === "failure")
							throw new Error("render admission unavailable");
						return {
				...
				await expect(
					campaignOperationService.renderSelected(request),
	...
	for (const action of ["motion", "style", "scene"] as const) {
		test(`Campaign ${action} reports a lost item settlement`, async () => {
			...
			await prisma.project.update({
				...
				data: { brandProfileId: profile.id },
			});
			await prisma.brandTemplate.create({
				data: {
					...
					workspaceId: current.workspace.id,
					captionPreset: {},
					profileMembership: { create: { profileId: profile.id } },
				},
			});
			const scene = await sceneTemplateService.create(scope, profile.id, {
				name: "Claim opener",
				role: "intro",
				makeDefault: false,
				definition: {
			...
			const style = (
				await campaignOperationService.getEditorActionCatalog(scope)
			).styles[0]!;
			const warnings = spyOn(console, "warn").mockImplementation(() => {});
			...
			try {
				...
				const request =
					action === "motion"
						...
						: action === "style"
							? campaignOperationService.applyStyleSelected(scope, {
									clips,
									templateId: style.id,
									templateFingerprint: style.fingerprint,
								})
							: campaignOperationService.applySceneTemplate(
									...
									scene.id,
									{
										clips,
										placement: "start",
										templateFingerprint: scene.fingerprint,
									},
								);
				await expect(request).rejects.toMatchObject({
					code: "campaign_operation_claim_lost",
				});
				expect(
					warnings.mock.calls.map(([value]) => JSON.parse(value)),
				).toContainEqual(
					expect.objectContaining({
						message: "campaign_operation_claim_lost",
						clipId: current.clip.id,
					}),
				);
			} finally {
	...
	for (const outcome of ["success", "failure"] as const) {
		test(`Campaign export bundle preserves the replacement owner's items after admission ${outcome}`, async () => {
			...
			const bundles = new CampaignOperationService(
				async ({ projectId, idempotencyKey, stage }) => {
					await prisma.campaignOperation.updateMany({
						where: { projectId },
						data: { claimToken: randomUUID() },
					});
	...
	for (const action of ["render", "export"] as const) {
		test(`Campaign ${action} reports claim loss when settling an empty selection`, async () => {
			...
			try {
				const request =
					action === "render"
						? campaignOperationService.renderSelected({
								...scope,
								clipIds: [randomUUID()],
								resolution: "1080p",
								execute: async () => {
						...
						: campaignOperationService.createExportBundle(scope, {
								clips: [{ clipId: randomUUID(), expectedEditorRevision: 3 }],
								aspectRatios: ["9:16"],
								resolution: "1080p",
							});
				await expect(request).rejects.toMatchObject({
				...
				expect(
					(await campaignOperationService.listOperations(scope))[0],
				).toMatchObject({ status: "running", claimToken: replacement });
			} finally {
				prismaGlobal.narriflowPrismaClient = prisma;
			}
		});
	...
	test("Autopilot's immutable token fences a late worker even when replacement lease dates match", async () => {
		...
		let replacementRun:
			| ReturnType<AutopilotService["processDueRules"]>
			| undefined;
		let originalToken: string | null = null;
		let replacementToken: string | null = null;
		const replacement = new AutopilotService({
			now: () => now,
			fetchFeed: async () => {
				replacementToken = (
		...
		const original = new AutopilotService({
			now: () => now,
			fetchFeed: async () => {
				originalToken = (
					await prisma.autopilotRule.findUniqueOrThrow({
		...
		try {
			expect(await original.processDueRules(1)).toEqual({
				checked: 1,
				imported: 0,
				claimLost: 1,
			});
			...
			release.resolve();
			expect(await replacementRun).toEqual({
				checked: 1,
				imported: 0,
				claimLost: 0,
			});
						...
						episodes: [
							{
								id: "one",
								title: "Episode one",
								enclosureUrl: "https://media.example.test/one.mp3",
								durationSeconds: 60,
								publishedAt: null,
								mimeType: "audio/mpeg",
							},
					...
					autopilotRule: {
						async updateMany({ args, query }) {
							...
							if (
								...
								const row = await prisma.autopilotRule.findUniqueOrThrow({
									where: { id: rule.id },
								});
	...
	test("an Autopilot edit revokes a claim acquired after the edit's initial read without stranding the rule", async () => {
		...
		const worker = new AutopilotService({
			now: () => workerNow,
			fetchFeed: async () => {
				entered.resolve();
				await release.promise;
		...
		try {
			...
			expect(await worker.processDueRules(1)).toMatchObject({
				checked: 1,
				claimLost: 0,
			});
	...
	test("a late atomic export handoff reports claim loss after a replacement has created the bundle", async () => {
		...
		let replacement:
			| Awaited<ReturnType<CampaignOperationService["createExportBundle"]>>
			| undefined;
		const extended = prisma.$extends({
			query: {
				campaignOperation: {
					async create({ args, query }) {
						...
						replacement = await campaignOperationService.createExportBundle(
							scope,
							input,
						);
						return operation;
		...
		try {
			await expect(
				campaignOperationService.createExportBundle(scope, input),
			).rejects.toMatchObject({ code: "campaign_operation_claim_lost" });
			expect(
				await campaignOperationService.listExportBundles(scope),
			).toMatchObject([{ workflowRunId: replacement!.workflowRunId }]);
			expect(
				(await campaignOperationService.listOperations(scope))[0]?.items[0]
					?.status,
			).toBe("pending");
		} finally {
			prismaGlobal.narriflowPrismaClient = prisma;
	...
	test("keeps assisted-copy prompts and content out of analytics metadata", async () => {
		...
		await assistedCopy.requireProvenance({
			workspaceId: current.workspace.id,
			projectId: current.project.id,
			clipId: current.clip.id,
			platform: "youtube_shorts",
	...
	test("lists every usable current-revision export so publishing can choose a compatible older render", async () => {
		...
		expect(exports.map((item) => item.id)).toEqual([
			newer.id,
			current.clipExport.id,
		]);
		expect(
			exports[1]?.variants.some(
	...
	test("rejects cross-project bulk clip identifiers before persisting an operation", async () => {
		...
		await expect(
			bulkSocialSchedulingService.schedule({
				actorUserId: current.user.id,
				workspaceId: current.workspace.id,
				projectId: current.project.id,
				value: {
		...
		expect(
			await prisma.campaignOperation.count({
	...
	test("settles duplicate and partial Render selected admission with stable item counts", async () => {
		...
		expect(
			operation.items
				.map((item) => [item.requestedClipId, item.status, item.errorCode])
				.sort(),
		).toEqual(
			[
	...
	test("persists revision-fenced motion with partial truth and idempotent replay", async () => {
		...
		const first = await campaignOperationService.applyMotionSelected(
			scope,
			request,
		);
		const replay = await campaignOperationService.applyMotionSelected(
			scope,
			request,
		);
		...
		await expect(
			...
			code: "campaign_operation_idempotency_conflict",
		});
	});
	...
	test("freezes bundle revisions and variants while excluding stale and missing clips", async () => {
		...
		const missingClipId = randomUUID();
		const bundleService = new CampaignOperationService(
			async ({ projectId, idempotencyKey, stage }) =>
		...
		expect(result.manifest.included[0]).toMatchObject({
			clipId: current.clip.id,
			editorRevision: 3,
		});
		expect(
			result.manifest.included[0]?.files.map((file) => file.variantId),
		).toEqual(current.clipExport.variants.map((variant) => variant.id));
		expect(result.manifest.excluded).toEqual([
			{ clipId: missingClipId, code: "campaign_clip_not_found" },
		]);
		expect(JSON.stringify(result.manifest)).not.toContain("http");

		await prisma.clip.update({
			where: { id: current.clip.id },
			data: { editorRevision: 4 },
		});
		await expect(
		...
		).rejects.toMatchObject({ code: "export_bundle_empty" });

		const exactRevision = await fixture("bundle-exact-export-revision");
		await prisma.clipExport.update({
			where: { id: exactRevision.clipExport.id },
			data: { editorRevision: 2 },
		});
		await expect(
			bundleService.createExportBundle(
				{
				...
				{
					clips: [{ clipId: exactRevision.clip.id, expectedEditorRevision: 3 }],
					aspectRatios: ["9:16"],
					resolution: "1080p",
				},
			),
	...
	test("retries an export bundle admission failure by source operation", async () => {
		...
		expect(source.items[0]).toMatchObject({
			status: "failed",
			errorCode: "export_bundle_admission_failed",
		});

		const succeeding = new CampaignOperationService(
			async ({ projectId, idempotencyKey, stage }) =>
				prisma.workflowRun.create({
					data: { projectId, idempotencyKey, stage, status: "queued" },
					select: { id: true },
		...
		const retry = await prisma.campaignOperation.findUniqueOrThrow({
			where: { id: retried.operationId },
		});
		expect(retry.retryOfId).toBe(source.id);
		expect(retried.manifest.included.map((item) => item.clipId)).toEqual([
			current.clip.id,
		]);
	});
	...
	test("allows only one concurrent retry and selects failed admission items", async () => {
		...
		const source = await prisma.campaignOperation.create({
			data: {
				workspaceId: current.workspace.id,
				projectId: current.project.id,
				actorUserId: current.user.id,
				...
				items: {
					create: {
						...
						status: "failed",
						errorCode: "campaign_render_admission_failed",
					},
				},
			},
		});
		...
		const outcomes = await Promise.allSettled(
			[0, 1].map(() =>
				campaignOperationService.retryRenderSelected({
					actorUserId: current.user.id,
					workspaceId: current.workspace.id,
					projectId: current.project.id,
					pricingTier: "business",
		...
		).toEqual([expect.any(CampaignOperationError)]);
		expect(
			await prisma.campaignOperation.count({ where: { retryOfId: source.id } }),
		).toBe(1);
	});
	...
	test("persists only the selected Facebook Page and consumes the selection once", async () => {
		...
		try {
			const callback = await socialOAuthService.handleCallback({
				state,
				code: "provider-code",
			...
			expect(
				await prisma.socialAccount.findMany({
					where: { userId: current.user.id, platform: "facebook_reels" },
					select: { providerAccountId: true },
				}),
			).toEqual([{ providerAccountId: "page-b" }]);
		} finally {
	...
	test("revokes replayed guest sessions, isolates media IDs, and serializes opposite decisions", async () => {
		...
		const persistedRound = await prisma.reviewRound.findUniqueOrThrow({
			where: { id: created.id },
		});
		expect(persistedRound.deliveryTokenEncrypted).not.toContain(created.token);
		expect(
			await prisma.reviewNotificationLedger.count({
				where: { reviewRoundId: created.id, kind: "round_sent" },
			}),
		).toBe(1);
		await expect(
		...
		expect(
			(
				await prisma.reviewRound.findUniqueOrThrow({
					where: { id: created.id },
				})
			).recipientEmails,
		).toEqual([
		...
		expect(
			await prisma.reviewAuditEvent.count({
				where: { reviewRoundId: created.id, kind: "reviewers_invited" },
			}),
		).toBe(3);
		await expect(
			reviewService.authenticate(
				created.token,
				{
					identity: "Client",
		...
		).rejects.toMatchObject({ code: "review_access_invalid" });
		const firstSession = await reviewService.authenticate(
			created.token,
			{
				identity: "Client",
				email: "client@example.test",
		...
		const secondSession = await reviewService.authenticate(
			created.token,
			{
				identity: "Client updated",
				email: "CLIENT@example.test",
				passcode: "review-secret",
			},
			"203.0.113.10",
		...
		for (let index = 0; index < 10; index += 1) {
			additionalSessions.push(
				await reviewService.authenticate(
					created.token,
					{
						identity: `Reviewer ${index + 1}`,
		...
		await expect(
			reviewService.readRound(firstSession, sessionSecret),
		).rejects.toMatchObject({ code: "review_session_invalid" });
		const snapshot = await reviewService.readRound(
			secondSession,
			sessionSecret,
		);
		const item = snapshot.round.items[0]!;
		await expect(
			reviewService.resolveMedia(
				secondSession,
				sessionSecret,
				item.id,
				foreign.clipExport.variants[0]!.id,
			),
		).rejects.toMatchObject({ code: "review_media_not_found" });
		await expect(
			reviewService.resolveMedia(
				secondSession,
				sessionSecret,
		...
		).rejects.toMatchObject({ code: "review_download_forbidden" });
		await expect(
			reviewService.addComment(secondSession, sessionSecret, {
				itemId: null,
				parentId: null,
				body: "Impossible round timecode",
				timestampSec: 1,
			}),
		).rejects.toMatchObject({ code: "review_timecode_requires_item" });
		await expect(
			reviewService.addComment(secondSession, sessionSecret, {
				itemId: item.id,
				parentId: null,
				body: "Past the frozen export",
				timestampSec: 11,
			}),
		).rejects.toMatchObject({ code: "review_timecode_out_of_range" });
		const parentComment = await reviewService.addComment(
			secondSession,
			sessionSecret,
			{
				itemId: item.id,
				parentId: null,
				body: "At the exact end",
				timestampSec: 10,
			},
		);
		const internalReplyParent = await reviewService.addComment(
			secondSession,
			sessionSecret,
			{
				itemId: item.id,
				parentId: null,
				body: "Please have the team respond",
				timestampSec: null,
			},
		);
		await reviewService.addInternalComment(
			...
			{
				itemId: item.id,
				parentId: internalReplyParent.id,
				body: "@client@example.test Please check this note",
				timestampSec: null,
				mentionRecipients: ["client@example.test"],
			},
		);
		expect(
			await prisma.reviewNotificationLedger.count({
				where: { reviewRoundId: created.id, kind: "mention" },
			}),
		).toBe(1);
		const concurrentEdits = await Promise.all([
			reviewService.editComment(
				secondSession,
				sessionSecret,
				parentComment.id,
				{ body: "Edit from browser A" },
			),
			reviewService.editComment(
				secondSession,
				sessionSecret,
				parentComment.id,
				{ body: "Edit from browser B" },
			),
		]);
		expect(concurrentEdits).toHaveLength(2);
		expect(["Edit from browser A", "Edit from browser B"]).toContain(
			(
		...
		expect(
			await prisma.reviewAuditEvent.count({
				where: {
					...
					kind: {
						in: ["comment_edited", "comment_resolved", "comment_reopened"],
					},
				},
			}),
		).toBe(4);
		const auditPayload = JSON.stringify(
			await prisma.reviewAuditEvent.findMany({
		...
		for (const secretValue of [
			...
			"fourth@example.test",
			parentComment.body,
			"https://review.example/private",
		]) {
			expect(auditPayload).not.toContain(secretValue);
		}
		const reply = await reviewService.addComment(
		...
		).rejects.toMatchObject({ code: "review_comment_has_replies" });
		await expect(
			reviewService.deleteComment(
				additionalSessions[0]!,
				sessionSecret,
		...
		const decisions = await Promise.all([
			reviewService.decide(secondSession, sessionSecret, {
				itemId: item.id,
				decision: "approved",
				reason: null,
			}),
		...
		expect(
			await prisma.reviewDecision.count({
				where: { reviewRoundId: created.id },
			}),
		).toBe(2);
		expect(
			await prisma.reviewDecision.count({
				where: { reviewRoundId: created.id, supersededAt: null },
			}),
		).toBe(1);
		await reviewService.decide(secondSession, sessionSecret, {
			itemId: item.id,
			decision: "changes_requested",
			reason: "Tighten the opening",
		});
		...
		expect(
			await prisma.reviewNotificationLedger.count({
				where: { reviewRoundId: created.id, kind: "first_change_requested" },
			}),
		).toBe(1);
		expect(
			await prisma.reviewNotificationLedger.count({
				where: { reviewRoundId: created.id, kind: "all_approved" },
			}),
		).toBe(1);
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_sent" },
			}),
		).toBe(1);
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_opened" },
			}),
		).toBe(1);
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_first_comment" },
			}),
		).toBe(1);
		expect(
			await prisma.projectAnalyticsEvent.count({
		...
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_item_approved" },
			}),
		).toBeGreaterThanOrEqual(1);
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "campaign_approved" },
			}),
		).toBe(1);
		...
		await expect(
			reviewService.readRound(secondSession, sessionSecret),
		).rejects.toBeInstanceOf(ReviewServiceError);
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_expired" },
			}),
		).toBe(1);
		await prisma.reviewRound.update({
			where: { id: created.id },
		...
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_revoked" },
			}),
		).toBe(1);
		await expect(
			reviewService.inviteReviewers(
	...
	test("allocates concurrent review revisions and freezes scene-template insertion", async () => {
		...
		expect(
			await prisma.reviewRound.count({
				where: { projectId: current.project.id, status: "open" },
			}),
		).toBe(1);
		...
		const unresolved = await reviewService.addComment(
			reviewSession,
			sessionSecret,
			{
				itemId: null,
		...
		expect(
			await prisma.reviewRoundContext.count({
				where: { reviewRoundId: linked.id, sourceCommentId: unresolved.id },
			}),
		).toBe(1);
		expect(
			(
				await reviewService.internalRoom(
					current.workspace.id,
					current.project.id,
					"manage",
				)
			...
			expect.objectContaining({
				sourceCommentId: unresolved.id,
				sourceRoundRevision: 3,
				body: unresolved.body,
			}),
		]);
		const viewerRoom = await reviewService.internalRoom(
			current.workspace.id,
			current.project.id,
			"view",
		);
		const viewerSourceRound = viewerRoom.rounds.find(
			(round) => round.id === resubmitted.id,
		);
		...
		expect(viewerSourceRound?.guests[0]).toMatchObject({
			displayName: null,
			email: null,
		});
		...
		expect(
			await prisma.projectAnalyticsEvent.count({
				where: { projectId: current.project.id, type: "review_resubmitted" },
			}),
		).toBe(4);
		...
		await expect(
			visualAssetService.assertSceneReferencesWithPolicy(
				scope,
				retainedScene,
				true,
			),
			...
			definition: {
				...
				content: {
					...
					fontAsset: {
						kind: "brand_font",
						id: sourceFont.id,
						fingerprint: sourceFont.fingerprint,
					},
					color: "#FFFFFF",
					backgroundColor: "#111827",
				},
		...
		await expect(
			brandFontService.softDelete(scope, sourceFont.id, {
				replacementId: replacementFont.id,
			}),
		...
		const frozen = await sceneTemplateService.freezeForInsertion(
			scope,
			profile.id,
			template.id,
			{ id: randomUUID(), anchorSec: 0 },
		);
		await sceneTemplateService.update(scope, profile.id, template.id, {
		...
		const once = applyEditorAction(document, {
			type: "insertSceneBlock",
			scene: frozen,
		});
		const twice = applyEditorAction(once, {
			type: "insertSceneBlock",
			scene: { ...frozen, id: randomUUID() },
		});
		expect(twice).toBe(once);

		const latestTemplate = await prisma.sceneTemplate.findUniqueOrThrow({
		...
		expect(reapplied.counts).toEqual({
			succeeded: 0,
			unchanged: 1,
			stale: 0,
			ineligible: 0,
		...
		const appliedAtEnd = await campaignOperationService.applySceneTemplate(
			...
			template.id,
			{
				...
				clips: [
					{
						clipId: current.clip.id,
						expectedEditorRevision: persisted.revision,
					},
				],
			},
		);
		...
		const reappliedAtEnd = await campaignOperationService.applySceneTemplate(
			...
			template.id,
			{
				...
				clips: [
					{
						clipId: current.clip.id,
						expectedEditorRevision: afterEnd.revision,
					},
				],
			},
		);
		...
		const createdExport = await clipExportService.create(
			...
			current.clip.id,
			{
				expectedRevision: finalDocument.revision,
				aspectRatios: ["9:16"],
				resolution: "1080p",
			},
			randomUUID(),
