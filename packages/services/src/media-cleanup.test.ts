...
describe("Media Cleanup", () => {
  ...
  test("admission can defer exact-key cleanup beyond an upload grant", async () => {
    const captured: Parameters<MediaCleanupAdmissionStore["createMany"]>[0][] = [];
    const nextAttemptAt = new Date("2026-08-29T01:05:00.000Z");
    const store: MediaCleanupAdmissionStore = {
      async createMany(input) {
        captured.push(input);
        return { count: input.data.length };
    ...
    await admitMediaCleanupObligations(store, [
      {
        origin: "visual_asset_upload",
        cleanupClass: "unfinalized_visual_asset_upload",
        objectKey: "workspaces/workspace-1/visual-assets/upload.png",
        nextAttemptAt,
      },
    ]);

    expect(captured[0]?.data[0]).toMatchObject({ nextAttemptAt });
  });

  test("unclaimed adoption is fenced before cleanup starts and idempotent after commit", async () => {
    let adopted = false;
    const identity = {
    ...
    const store = {
      async updateMany(input: { where: { attemptCount: number; claimId: null } }) {
        expect(input.where).toMatchObject({ attemptCount: 0, claimId: null });
        if (adopted) return { count: 0 };
        adopted = true;
        return { count: 1 };
      },
      async count(input: { where: { failureCode: string } }) {
        expect(input.where.failureCode).toBe("visual_asset_adopted");
        return adopted ? 1 : 0;
      },
    };
    ...
    await expect(
      adoptUnclaimedMediaCleanupObligation(
        store,
        identity,
        new Date("2026-08-29T00:30:01.000Z"),
        "visual_asset_adopted",
      ),
    ).resolves.toBe("replayed");
  });

  test("unclaimed adoption rejects an active or previously attempted cleanup", async () => {
    await expect(
      adoptUnclaimedMediaCleanupObligation(
        {
          async updateMany() {
        ...
        new Date("2026-08-29T01:06:00.000Z"),
        "visual_asset_adopted",
      ),
    ).rejects.toBeInstanceOf(MediaCleanupAdoptionLost);
  });

  test("held adoption requires the live producer claim and preserves a durable receipt", async () => {
    let adopted = false;
    const identities = [
      {
        origin: "export_bundle_attempt" as const,
        cleanupClass: "export_bundle_unsettled_publication" as const,
        objectKey: "projects/project-1/campaign-operations/operation-1/completed/attempt-1.zip",
      },
    ];
    const store = {
      async updateMany(input: { where: { claimId: string; claimExpiresAt: { gt: Date } } }) {
        expect(input.where.claimId).toBe("attempt-1");
        expect(input.where.claimExpiresAt.gt).toEqual(
      ...
      async count(input: { where: { failureCode: string } }) {
        expect(input.where.failureCode).toBe("export_bundle_published");
        return adopted ? 1 : 0;
      },
    };
  ...
  test("does not claim an unfinalized upload until its URL lifetime and skew buffer end", async () => {
    let now = new Date("2026-08-29T01:00:00.000Z");
    const cleanupDueAt = new Date("2026-08-29T01:05:00.000Z");
    const objectKey = "workspaces/workspace-1/visual-assets/upload.png";
    const store = createInMemoryMediaCleanupStore([
      seed({
        id: "visual-upload-cleanup",
        origin: "visual_asset_upload",
        projectId: null,
        clipId: null,
        cleanupClass: "unfinalized_visual_asset_upload",
  ...
  test("reaps only orphaned attempt-scoped bundle objects after the producer hold expires", async () => {
    const now = new Date("2026-08-31T00:00:00.000Z");
    const expiredAt = new Date(now.getTime() - 1);
    const attemptKey =
      "projects/project-1/campaign-operations/operation-1/attempts/attempt-1.zip";
    const unsettledFinalKey =
      "projects/project-1/campaign-operations/operation-1/completed/attempt-1.zip";
    const adoptedFinalKey =
      "projects/project-1/campaign-operations/operation-1/completed/attempt-2.zip";
    const store = createInMemoryMediaCleanupStore([
      seed({
        id: "bundle-attempt",
        ...
        objectKey: attemptKey,
        claimId: "attempt-1",
        claimExpiresAt: expiredAt,
      }),
      seed({
        id: "bundle-unsettled-final",
        origin: "export_bundle_attempt",
        projectId: "project-1",
        clipId: null,
        cleanupClass: "export_bundle_unsettled_publication",
        objectKey: unsettledFinalKey,
        claimId: "attempt-1",
        claimExpiresAt: expiredAt,
      }),
      seed({
        id: "bundle-adopted-final",
        origin: "export_bundle_attempt",
        projectId: "project-1",
        clipId: null,
        cleanupClass: "export_bundle_unsettled_publication",
        objectKey: adoptedFinalKey,
        failureCode: "export_bundle_published",
        completedAt: new Date("2026-08-30T23:00:00.000Z"),
      }),
    ]);
    const deleted: string[] = [];
    const worker = createMediaCleanupWorker({
