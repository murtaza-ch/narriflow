...
function toItem(row: OperationRow["items"][number]): BulkScheduleItem {
  const result = storedResult(row.result);
	...
	if (!clipId || row.expectedEditorRevision === null || !result.requestKey || !result.accountId || !result.platform) {
...
async function readOperation(workspaceId: string, projectId: string, idempotencyKey: string) {
...
export const prismaBulkScheduleStore: BulkScheduleStore = {
  async open(input) {
    const existing = await readOperation(input.workspaceId, input.projectId, input.idempotencyKey);
    ...
    try {
      ...
      const raced = await readOperation(input.workspaceId, input.projectId, input.idempotencyKey);
  ...
  async claimItem(operationId, requestKey) {
    ...
    const now = new Date();
    const claimToken = bulkScheduleDeterministicUuid(`${operationId}:${requestKey}:${now.toISOString()}`);
    ...
    const row = operation.items.find((item) => item.requestedClipId === requestedClipId);
  ...
  async settleItem(operationId, requestKey, claimToken, patch) {
    ...
    await requirePrisma().$transaction(async (tx) => {
			const current = await tx.campaignOperationItem.findFirst({
				where: { operationId, requestedClipId, status: "processing", claimToken },
      });
			if (!current) throw new BulkSocialSchedulingError("campaign_schedule_claim_lost");
      ...
      const frozen = patch.socialPostId
        ...
        data: {
          ...
          reviewApprovalOverrideId: frozen?.reviewApprovalOverrideId ?? current.reviewApprovalOverrideId,
          result: {
            ...result,
  ...
  async settleOperation(operationId, now) {
    return requirePrisma().$transaction(async (tx) => {
      ...
      const operation = await tx.campaignOperation.update({
        where: { id: operationId },
        data: {
          status: succeeded === 0 ? "failed" : ineligible + failed > 0 ? "partial" : "completed",
...
function productionModule() {
  return createBulkSocialScheduling({
    ...
    authorize: async ({ actorUserId, workspaceId, projectId, clipIds, permission }) => {
...
export class BulkSocialSchedulingService {
  schedule(input: {
    ...
    const parsed: BulkSocialScheduleRequest = bulkSocialScheduleSchema.parse(input.value);
