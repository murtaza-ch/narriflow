...
function configuredDatabaseUrl() {
  const configured =
    process.env.SOCIAL_PUBLICATION_TEST_DATABASE_URL ??
...
const schema =
  process.env.SOCIAL_PUBLICATION_TEST_DATABASE_SCHEMA ??
  `social_publication_test_${randomUUID().replaceAll("-", "")}`;
if (!/^social_publication_test_[a-z0-9_]+$/.test(schema)) {
  throw new Error("Unsafe Social Publication test schema name");
}
...
try {
  ...
  await run(["bun", "test", "packages/services/src/social-publication.db.test.ts"], {
    ALLOW_SOCIAL_PUBLICATION_DB_TESTS: "1",
    DATABASE_URL: testUrl.toString(),
    DIRECT_URL: testUrl.toString(),
    SOCIAL_PUBLICATION_TEST_DATABASE_URL: testUrl.toString(),
    SOCIAL_PUBLICATION_TEST_DATABASE_SCHEMA: schema,
  });
} finally {
  if (process.env.SOCIAL_PUBLICATION_TEST_KEEP_SCHEMA !== "1") {
