...
The Clip table remains the storage projection. One codec applies current null defaults, rejects malformed non-null document values, owns canonical equality, and encodes validated JSON for Prisma. Every real mutation increments the revision once, retires mutable Clip Render rows, and commits document columns, first-original capture, derived-state invalidation, and deduplicated cleanup obligations in one transaction. Immutable Clip Exports remain unchanged. A separate worker claims exact-object cleanup obligations with expiring fenced claims; storage failure never reverses an accepted edit.
