...
The public interface starts and resumes Checkout, observes Checkout returns, opens the portal, accepts a delivery, reconciles current or due accounts, reads a product billing view, and provides identifier-safe operator inspection. The production Stripe adapter retrieves the customer and the complete bounded subscription collection. Personal Workspaces fall back to active Free when paid access ends. Collaborative Workspaces with non-owner members become restricted on Free. Restricted access preserves owner billing, viewing, downloads, Viewer invitations, demotions, and removals while blocking edits, processing, publishing, API and MCP use, and new billable collaboration.

## Completion review — 2026-08-28

The direct pre-production cutover is complete. The old subscription page, optimistic return confirmation, direct tier application, foreground seat calls, fixed first-page seat sweep, compatibility fields, and User billing inputs are absent. Replayable Checkout/customer provisioning and committed-membership seat convergence are durable Workspace Billing phases. Verification is reproducible through the module and HTTP contract suites, pinned Stripe signature fixtures, live test-mode Stripe contracts, the full disposable PostgreSQL migration/recovery drill, repository typecheck/lint/test/build, and authenticated Chrome checks documented in the Workspace Billing runbook.
