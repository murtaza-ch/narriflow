...
Clip duplication admits provisional obligations before starting any remote copy. Those obligations remain under a heartbeat-renewed producer claim while copies are in flight. Immediately before adoption the producer renews once more; the duplicate transaction completes every successful copy's obligation with an adoption-specific receipt only under that still-unexpired claim and rolls back if the fenced count differs. A repeated adoption accepts only that receipt, never an ordinary cleanup completion. Failed, ambiguous, or unadopted destinations are released for Media Cleanup, and release diagnostics verify the fenced row count. A producer crash or claim takeover therefore leaves each destination either referenced by the committed duplicate or recoverable by Media Cleanup, never both.
