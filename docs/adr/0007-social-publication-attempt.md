...
YouTube resumes its provider-reported byte range. Instagram resumes the same container. TikTok releases moderation to scheduled status checks and verified, replay-bounded webhooks keyed to the same `publish_id`. LinkedIn and X perform bounded exact media-identity lookups when the connected account grants read access and otherwise surface explicit Needs attention rather than retrying a create request.
