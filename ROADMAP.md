...
- **RSS autopilot + MCP server** _(June–August 2026)_: durable RSS autopilot rules watch feeds, dedupe episode IDs, import unseen episodes, persist generation context before ingest completion, and then flow through the normal generation pipeline. Narriflow exposes a stateless MCP `2026-07-28` Streamable HTTP endpoint with Clerk OAuth and scoped workspace API keys, plus an API-key-authenticated stdio fallback. It supports project, usage, and autopilot tools across Codex, Claude, and compatible clients.
...
9. **Feed-driven autopilot + stateless MCP server** — **[DIFF] medium.** ✅ RSS feed autopilot shipped with episode dedupe and saved generation settings; ✅ MCP `2026-07-28` remote HTTP and local stdio transports shipped with OAuth/API-key auth, workspace authorization, billing gates, usage visibility, and project/autopilot tools. **Remaining:** YouTube channel watcher and MCP tools for fine-grained studio edits.
